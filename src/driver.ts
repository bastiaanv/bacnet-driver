...
import { NetworkProtocolDataUnit } from './npdu';
...
export class BacnetDriver {
   ...
   * @fires iAm
   */
  public whoIs(address?: string): void {
    ...
    // Send message
    this.transporter.send(buffer, address || '255.255.255.255');
  }
  ...
  private getDevice(address: string, deviceId: number): FoundDevice {
    ...
    if (!device) {
      throw new Error(
        'This device has not been found in the bacnet network, therefore a message could not been send. Make sure to call the WhoIs first, before making a request'
      );
    }
  ...
  private receiveData(buffer: Buffer, address: string): void {
    ...
    // Check BVLC function
    const validBvlcFunctions: number[] = [
  ...
  private processSegment(
    ...
    this.performDefaultSegmentHandling(
      lastSequenceIndex,
      receiver,
      type,
      service,
  ...
  private segmentAckResponse(
    ...
    const buffer = this.getBuffer();
    NetworkProtocolDataUnit.encode(
      buffer,
      NpduControlPriority.NORMAL_MESSAGE,
      { macLayerAddress: ndpu.macLayerAddress, networkAddress: ndpu.networkAddress },
      0xff,
      false
    );
    ApplicationProtocolDataUnit.encodeSegmentAck(
      buffer,
      PduTypes.SEGMENT_ACK | (negative ? PduSegAckBits.NEGATIVE_ACK : 0) | 0,
      originalInvokeId,
      sequencenumber,
      actualWindowSize
    );
  ...
  private performDefaultSegmentHandling(
    segmentIndex: number,
    adr: string,
    type: number,
    service: number,
    ...
    if (first) {
      ...
      if ((type & PDU_TYPE_MASK) === PduTypes.CONFIRMED_REQUEST) {
        ApplicationProtocolDataUnit.encodeConfirmedServiceRequest(
          apdubuffer,
          service,
          invokeId,
          this.segmentStore[segmentIndex].lastNumber,
          proposedWindowNumber,
          true
        );
      } else {
  ...
  private addCallback(id: number, resolve: (data: any) => void, reject: (message: string) => void) {
    const timeout = setTimeout(() => {
      this.invokeStore.splice(
        this.invokeStore.findIndex((x) => x.invokeId === id),
        1
      );
